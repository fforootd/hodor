...
if (missing.length > 0) {
  console.error(
    `[e2e] Missing workspace dependencies: ${missing.join(', ')}.\n` +
      `[e2e] Run "npm ci" in ${rootDir} to refresh workspace installs before starting Playwright.`,
  )
  process.exit(1)
}
...
async function ensurePortAvailable(target) {
  ...
  await new Promise((resolve, reject) => {
    ...
    console.error(
      `[e2e] ${target.label} cannot start on ${hostname}:${port} because ${reason}.\n` +
        `[e2e] Free the port or run the smoke suite in an environment that permits local listeners before starting Playwright.`,
    )
    process.exit(1)
  })
}
