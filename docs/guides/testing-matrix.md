...
Current OIDC browser gap: [browser-tests/journeys/login/oidc-code-pkce.spec.ts](browser-tests/journeys/login/oidc-code-pkce.spec.ts) still has one failing `prompt=login` path in the real `auth_request_id` UI flow. The repo still enforces `prompt=login` semantics at the server layer in [crates/zitadel-server/tests/contracts_http_router.rs](crates/zitadel-server/tests/contracts_http_router.rs), but the browser journey is currently blocked by the login page not advancing from the password step after the forced re-auth prompt.
