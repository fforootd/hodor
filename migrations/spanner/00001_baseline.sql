...
CREATE TABLE IF NOT EXISTS instances (
    ...
    FOREIGN KEY (parent_instance_id) REFERENCES instances(instance_id) ON DELETE NO ACTION
);
...
CREATE TABLE IF NOT EXISTS users (
    ...
    updated_at    TIMESTAMP NOT NULL DEFAULT (CURRENT_TIMESTAMP()),
    PRIMARY KEY (instance_id, id),
    FOREIGN KEY (instance_id, org_id) REFERENCES orgs(instance_id, id)
);
...
CREATE TABLE IF NOT EXISTS apps (
    ...
    post_logout_redirect_uris  STRING(MAX) DEFAULT ('[]'),
    state                      STRING(MAX) NOT NULL DEFAULT ('active'),
    schema_id       STRING(MAX) DEFAULT (''),
    metadata        STRING(MAX) DEFAULT ('{}'),
    created_at      TIMESTAMP NOT NULL DEFAULT (CURRENT_TIMESTAMP()),
