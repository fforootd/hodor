...
impl AuthzService {
    pub fn new() -> Self {
        ...
        tracing::info!(policies = policies.policies().count(), "Cedar policies loaded");
    ...
    pub fn check(&self, user_id: &str, action: &str, resource_type: &str, resource_id: &str) -> AuthzDecision {
        // Fast path: instance owners.
        if self.is_instance_owner(user_id) {
            return AuthzDecision { allowed: true, reason: "instance_owner".into() };
        }

        // Build Cedar request.
        let principal = match EntityUid::from_str(&format!("User::\"{}\"", user_id)) {
            Ok(p) => p,
            Err(_) => return AuthzDecision { allowed: false, reason: "invalid_principal".into() },
        };
        let action_uid = match EntityUid::from_str(&format!("Action::\"{}\"", action)) {
            Ok(a) => a,
            Err(_) => return AuthzDecision { allowed: false, reason: "invalid_action".into() },
        };
        let resource = match EntityUid::from_str(&format!("{}::\"{}\"", capitalize(resource_type), resource_id)) {
            Ok(r) => r,
            Err(_) => return AuthzDecision { allowed: false, reason: "invalid_resource".into() },
        };

        let request = match Request::new(principal, action_uid, resource, Context::empty(), None) {
            Ok(r) => r,
            Err(e) => return AuthzDecision { allowed: false, reason: format!("request_error: {e}") },
        };
        ...
        AuthzDecision {
            ...
            reason: if response.decision() == Decision::Allow { "policy_allow".into() } else { "default_deny".into() },
        }
    }
}
