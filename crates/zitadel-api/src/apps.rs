...
async fn get_one(State(s): State<ApiState>, Path(id): Path<String>) -> Response {
    match get_named_resource(&s.db, current_instance_id().as_ref(), "apps", &id).await {
...
async fn update(
    ...
    .await
    {
        ...
        Ok(false) => response::not_found("app not found"),
        Err(e) => response::internal_error(format!("{e}")),
    }
}

async fn delete_one(State(s): State<ApiState>, Path(id): Path<String>) -> Response {
    match delete_instance_row(&s.db, current_instance_id().as_ref(), "apps", &id).await {
