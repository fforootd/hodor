...
async fn create(State(s): State<ApiState>, Json(req): Json<CreateRequest>) -> Response {
    ...
    match sqlx::query(
        "INSERT INTO groups (id, instance_id, name, state) VALUES ($1, $2, $3, 'active')",
    )
    .bind(&id)
...
async fn get_one(State(s): State<ApiState>, Path(id): Path<String>) -> Response {
    ...
    let sql = format!(
        "SELECT id, name, state, {created_at}, {updated_at} FROM groups WHERE instance_id = $1 AND id = $2"
    );
...
async fn list(
    ...
    let (created_at, updated_at) = scoped.select_timestamps();
    let sql = format!(
        "SELECT id, name, state, {created_at}, {updated_at} FROM groups WHERE instance_id = $1 AND id > $2 ORDER BY id LIMIT $3"
    );
    match sqlx::query_as::<_, (String, String, String, String, String)>(&sql)
        .bind(scoped.instance_id())
        .bind(&cursor)
...
async fn update(
    ...
    let result = sqlx::query("UPDATE groups SET name = $1, updated_at = CURRENT_TIMESTAMP WHERE instance_id = $2 AND id = $3")
        .bind(&req.name).bind(scoped.instance_id()).bind(&id).execute(scoped.pool()).await;
    response::handle_mutation(result, "group", || {
...
async fn delete_one(State(s): State<ApiState>, Path(id): Path<String>) -> Response {
    response::delete_by_id(&s.db.scoped_default(), "groups", &id, "group").await
}
