...
async fn create(
    ...
    match s
        .app
        .runner
        .run(&ctx, "group.create", || {
            s.app.create_group.execute(&ctx, cmd)
        })
        .await
    {
...
async fn get_one(
    ...
    match s
        .app
        .runner
        .run(&ctx, "group.get", || s.app.get_group.execute(&ctx, &id))
        .await
    {
...
async fn list(
    ...
    match s
        ...
        .runner
        .run(&ctx, "group.list", || {
            s.app.list_groups.execute(&ctx, None, &params)
        })
        .await
    {
        Ok(result) => {
...
async fn update(
    ...
    match s
        .app
        .runner
        .run(&ctx, "group.update", || {
            s.app.update_group.execute(&ctx, cmd)
        })
        .await
    {
        Ok(group) => response::json_ok(ItemResponse::from(group)),
...
async fn delete_one(
    ...
    match s
        .app
        .runner
        .run(&ctx, "group.delete", || {
            s.app.delete_group.execute(&ctx, &id)
        })
        .await
    {
