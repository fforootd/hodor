...
use hodor_auth::password::{Passwords, decode_credential_json};
use hodor_db::Db;
...
pub struct LoginState {
    ...
    pub cookie_config: Arc<hodor_auth::cookie::CookieConfig>,
}
...
async fn login(State(state): State<LoginState>, Json(req): Json<LoginRequest>) -> Response {
    ...
    // Create session.
    let session_store = hodor_auth::session::SessionStore::new(state.db.clone());
