...
fn parse_spanner_column_defaults(
    ...
    for statement in statements {
        ...
        for line in body.lines() {
            ...
            if line.contains(" GENERATED BY DEFAULT AS IDENTITY") {
                continue;
            }
...
fn parse_spanner_foreign_keys(statements: &[String]) -> BTreeMap<String, Vec<ForeignKeyManifest>> {
    ...
    for statement in statements {
        ...
        if trimmed.to_ascii_uppercase().starts_with("CREATE TABLE") {
            ...
            for line in body.lines() {
                let line = line.trim().trim_end_matches(',').trim();
                if let Some(foreign_key) = parse_spanner_foreign_key_clause(line) {
        ...
        let rest = rest.trim();
        let Some((table_name_raw, after_table_name)) = rest.split_once("ADD CONSTRAINT") else {
            continue;
        };
        let table_name = table_name_raw.trim().trim_matches('`').to_string();
        let Some((_, clause)) = after_table_name.trim().split_once("FOREIGN KEY") else {
            continue;
        };
        let clause = format!("FOREIGN KEY{}", clause.trim_end_matches(';'));
        if let Some(foreign_key) = parse_spanner_foreign_key_clause(&clause) {
            tables.entry(table_name).or_default().push(foreign_key);
        }
    }
...
fn parse_spanner_foreign_key_clause(clause: &str) -> Option<ForeignKeyManifest> {
    let line = clause.trim().trim_end_matches(',').trim();
    let normalized = if line.starts_with("CONSTRAINT") {
        let (_, rest) = line.split_once("FOREIGN KEY")?;
        format!("FOREIGN KEY{rest}")
    } else {
        line.to_string()
    };
    ...
    let (left, right) = normalized.split_once("REFERENCES")?;
    let local_columns_raw = left
    ...
    let (referenced_table_raw, referenced_rest) = referenced.split_once('(')?;
    let (referenced_columns_raw, after_columns) = referenced_rest.split_once(')')?;
...
fn reconcile_spanner_foreign_keys_with_canonical(
    table_name: &str,
    foreign_keys: Vec<ForeignKeyManifest>,
) -> Vec<ForeignKeyManifest> {
    let Some(canonical_table) = canonical_table_manifest(table_name) else {
        return foreign_keys;
    };
    ...
    foreign_keys
        .into_iter()
        .map(|mut foreign_key| {
            if let Some(canonical_foreign_key) =
                canonical_table.foreign_keys.iter().find(|expected| {
                    expected.columns == foreign_key.columns
                        && expected.referenced_table == foreign_key.referenced_table
                        && expected.referenced_columns == foreign_key.referenced_columns
                })
            {
                foreign_key.on_delete = canonical_foreign_key.on_delete;
            }
            foreign_key
        })
        .collect()
    ...
    let mut columns = match db.dialect() {
        ...
        Dialect::Spanner => {
            ...
            while let Some(row) = rows.next().await? {
                ...
                let family = logical_type_family(table_name, &name, &raw_type);
                let canonical_pk = canonical_table_manifest(table_name)
                    .is_some_and(|table| table.primary_key.iter().any(|pk| pk == &name));
...
async fn list_foreign_keys(
    ...
    let mut foreign_keys = match db.dialect() {
        ...
        Dialect::Spanner => reconcile_spanner_foreign_keys_with_canonical(
            table_name,
            spanner_foreign_keys
                .and_then(|tables| tables.get(table_name))
                .cloned()
                .unwrap_or_default(),
        ),
...
mod tests {
    ...
    #[test]
    fn canonical_jobs_enabled_stays_boolean() {
        let jobs = canonical_table_manifest("jobs").expect("jobs table");
        let enabled = jobs
            .columns
            .iter()
        ...
        assert_eq!(enabled.family, ColumnTypeFamily::Bool);
        assert_eq!(enabled.default, Some(DefaultValueManifest::Bool(true)));
    }
    ...
    #[test]
    fn parse_spanner_foreign_keys_handles_alter_table_constraints() {
        let statements = vec![
            ...
            .to_string(),
            "ALTER TABLE credentials ADD CONSTRAINT fk_credentials_example FOREIGN KEY (instance_id, user_id) REFERENCES users(instance_id, id) ON DELETE CASCADE;"
                .to_string(),
        ];

        let parsed = parse_spanner_foreign_keys(&statements);
        let credentials = parsed.get("credentials").expect("credentials table");
        assert_eq!(
            credentials,
            &vec![ForeignKeyManifest {
                columns: vec!["instance_id".into(), "user_id".into()],
                referenced_table: "users".into(),
                referenced_columns: vec!["instance_id".into(), "id".into()],
    ...
    #[test]
    fn parse_spanner_foreign_keys_handles_constraint_before_foreign_key() {
        let statements = vec![
            "CREATE TABLE credentials (
                instance_id STRING(MAX) NOT NULL,
                id STRING(MAX) NOT NULL,
                user_id STRING(MAX) NOT NULL,
                CONSTRAINT fk_credentials_example FOREIGN KEY(instance_id, user_id) REFERENCES users(instance_id, id) ON DELETE CASCADE,
                PRIMARY KEY(instance_id, id)
            );"
                .to_string(),
        ];
        ...
        assert_eq!(credentials[0].columns, vec!["instance_id", "user_id"]);
        assert_eq!(credentials[0].referenced_table, "users");
    }

    #[test]
    fn parse_spanner_column_defaults_ignores_identity_generation() {
        let statements = vec![
            "CREATE TABLE fga_tuple_changes (
                seq INT64 GENERATED BY DEFAULT AS IDENTITY (BIT_REVERSED_POSITIVE),
        ...
        let parsed = parse_spanner_column_defaults(&statements);
        let defaults = parsed
            .get("fga_tuple_changes")
            .expect("fga_tuple_changes table");
        assert!(!defaults.contains_key("seq"));
        assert_eq!(
            defaults.get("created_at"),
            Some(&"(CURRENT_TIMESTAMP())".to_string())
        );
    }

    #[test]
    fn spanner_foreign_keys_reconcile_backend_delete_downgrades_to_canonical_shape() {
        let reconciled = reconcile_spanner_foreign_keys_with_canonical(
            "instances",
            vec![
                ForeignKeyManifest {
                    columns: vec!["parent_instance_id".into()],
                    referenced_table: "instances".into(),
                    referenced_columns: vec!["instance_id".into()],
                    on_delete: ForeignKeyDeleteAction::NoAction,
                },
                ForeignKeyManifest {
                    columns: vec!["parent_instance_id".into(), "owner_org_id".into()],
        ...
        assert_eq!(reconciled[0].on_delete, ForeignKeyDeleteAction::Cascade);
        assert_eq!(reconciled[1].on_delete, ForeignKeyDeleteAction::Restrict);
    }
    ...
    async fn spanner_ddl_parsing_keeps_credentials_foreign_key_when_configured() {
        ...
        crate::migrate::migrate(&db)
            .await
            .expect("migrate spanner db");
        let ddl = db
            .spanner()
            .expect("native spanner client")
            .current_ddl()
            .await
            .expect("fetch current ddl");
        let foreign_keys = parse_spanner_foreign_keys(&ddl);
        let credentials = foreign_keys.get("credentials");
        assert!(
            credentials.is_some_and(|foreign_keys| foreign_keys.iter().any(|fk| {
                fk.columns == vec!["instance_id", "user_id"]
                    && fk.referenced_table == "users"
                    && fk.referenced_columns == vec!["instance_id", "id"]
                    && fk.on_delete == ForeignKeyDeleteAction::Cascade
            })),
            "parsed credentials foreign keys: {credentials:?}\nDDL:\n{}",
            ddl.join("\n\n")
        );
    }
