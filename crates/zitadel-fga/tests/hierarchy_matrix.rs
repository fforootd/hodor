...
async fn run_hierarchy_smoke(db: Db) -> anyhow::Result<()> {
...
#[tokio::test]
async fn hierarchy_smoke_works_on_sqlite() -> anyhow::Result<()> {
    let db = Db::open("").await?;
    run_hierarchy_smoke(db).await
}

#[tokio::test]
async fn hierarchy_smoke_works_on_postgres_when_configured() -> anyhow::Result<()> {
