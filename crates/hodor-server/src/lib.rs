...
pub fn build_router(state: Arc<AppState>, api_state: hodor_api::ApiState, oidc_state: hodor_oidc::OidcState, login_state: hodor_login::LoginState) -> Router {
    Router::new()
        // Health probes
        .merge(health::routes(state.clone()))
        // OIDC provider (discovery, authorize, token, userinfo, JWKS)
        .merge(hodor_oidc::routes(oidc_state))
        // Login flows (public, no auth required)
        .merge(hodor_login::routes(login_state))
        // REST API (/v1/*)
        .merge(hodor_api::routes(api_state))
...
pub async fn run(config: Config) -> anyhow::Result<()> {
    ...
    // Open database.
    let db = hodor_db::Db::open_with_config(&config.database.url, &config.database).await?;
    ...
    match config.database.resolve_migrate_mode() {
        "auto" => hodor_db::migrate::migrate(&db).await?,
        "check" => hodor_db::migrate::check_version(&db).await?,
        "skip" => tracing::info!("migration skipped"),
        _ => hodor_db::migrate::migrate(&db).await?,
    }

    // Bootstrap (create default org + admin if empty).
    if config.database.resolve_bootstrap_mode() == "auto" {
        hodor_db::bootstrap::bootstrap(&db).await?;
    }
    ...
    if !config.dev.seed_file.is_empty() {
        ...
        if seed_path.exists() {
            hodor_db::seed::apply(&db, seed_path).await?;
        } else {
    ...
    // Build cookie config.
    let cookie_config = hodor_auth::cookie::CookieConfig::new(
    ...
    let passwords = if config.is_dev() {
        hodor_auth::password::Passwords::new_dev()
    } else {
        hodor_auth::password::Passwords::new()
    };

    // OIDC provider.
    let issuer = format!("http://{}:{}", config.server.external_domain, config.server.port);
    let oidc_state = hodor_oidc::OidcState::new(db.clone(), issuer).await?;

    let api_state = hodor_api::ApiState {
        db: db.clone(),
    ...
    let login_state = hodor_login::LoginState {
        db: db.clone(),
