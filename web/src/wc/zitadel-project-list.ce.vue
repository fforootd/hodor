  ...
  <div class="zitadel-wc" :class="{ dark: isDark }">
    <div class="space-y-5">
      ...
      <div v-if="showCreateForm" class="card create-form animate-slide-in">
        <h3 class="text-sm font-semibold mb-3">Create Project</h3>
        <div class="space-y-3">
          <div class="space-y-1">
            <label class="text-sm font-medium">Name <span class="text-red-500">*</span></label>
            <input v-model="newName" type="text" placeholder="my-app" class="input-field" @keyup.enter="createProject" />
          </div>
          <div class="space-y-1">
            <label class="text-sm font-medium">Description</label>
            <input v-model="newDesc" type="text" placeholder="Customer-facing web application" class="input-field" />
          </div>
          <div v-if="createError" class="error-banner">{{ createError }}</div>
          <button class="btn-primary w-full" :disabled="!newName.trim() || creating" @click="createProject">
      ...
      <!-- Search -->
      <div v-if="showSearch && projects.length > 0" class="relative">
        ...
        <input v-model="searchQuery" type="text" placeholder="Search projects…" class="input-field pl-9" />
      </div>
      ...
      <div v-if="!loading && filteredProjects.length" class="rounded-lg border border-[var(--color-border)] overflow-hidden">
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b bg-[var(--color-muted)]">
              <th class="h-10 px-4 text-left font-medium text-[var(--color-muted-foreground)]">Project</th>
          ...
          <tbody>
            <tr v-for="p in filteredProjects" :key="p.id" class="table-row-hover" @click="onProjectClick(p)">
              <td class="p-4">
              ...
              <td class="p-4 text-[var(--color-muted-foreground)] truncate max-w-[200px]">{{ p.description || '—' }}</td>
              <td class="p-4"><span class="state-badge state-active">{{ p.member_count ?? 0 }}</span></td>
              <td class="p-4">
                <span :class="['state-badge', p.state === 'active' ? 'state-active' : 'state-inactive']">
                  {{ p.state || 'active' }}
                </span>
              </td>
            </tr>
      ...
      <div v-if="!loading && !error && !filteredProjects.length" class="empty-state">
        <div class="empty-icon">📁</div>
        <p class="text-sm font-medium">{{ searchQuery ? 'No projects match your search.' : 'No projects yet.' }}</p>
        <p v-if="!searchQuery" class="text-xs text-[var(--color-muted-foreground)] mt-1">Create your first project to organize apps and resources.</p>
      </div>
    </div>
  </div>
</template>
...
const TAG_NAME = 'zitadel-project-list'
...
function onProjectClick(project: any) {
  dispatchWCEvent(TAG_NAME, 'project-selected', { id: project.id, name: project.name })
}
...
async function createProject() {
  ...
  createError.value = ''
  try {
    const result = await api.post<any>('/v1/projects', { name: newName.value.trim(), description: newDesc.value.trim() })
    dispatchWCEvent(TAG_NAME, 'project-created', { id: result.id, name: result.name })
    newName.value = ''
    newDesc.value = ''
    ...
    dispatchWCEvent(TAG_NAME, 'project-error', { error: createError.value })
  } finally {
...
async function loadProjects() {
  ...
  error.value = ''
  try {
    const data = await api.get<any>('/v1/projects')
