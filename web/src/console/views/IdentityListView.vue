    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-muted-foreground">
          {{ identities.length }} {{ label.toLowerCase() }} total
        </p>
      </div>
      <Button as-child>
            ...
            >
              <div class="py-1">
                <div v-if="!currentFilterPrefix">
                  ...
                  <button
                    class="w-full text-left px-2 py-1.5 text-sm hover:bg-muted cursor-pointer flex items-center transition-colors"
                    @mousedown.prevent="appendSearchToken('identifier:')"
                  >
                    <span class="font-medium mr-2">identifier:</span> Search by Identifier
                  </button>
                  <button
                    class="w-full text-left px-2 py-1.5 text-sm hover:bg-muted cursor-pointer flex items-center transition-colors"
                    @mousedown.prevent="appendSearchToken('state:')"
                  >
                ...
                <div v-if="currentFilterPrefix === 'state:'">
                  ...
                  <button
                    ...
                    @mousedown.prevent="appendSearchToken('state:active ')"
                  >
                    Active
                  </button>
                  <button
                    class="w-full text-left px-2 py-1.5 text-sm hover:bg-muted cursor-pointer transition-colors"
                    @mousedown.prevent="appendSearchToken('state:deactivated ')"
                  >
                    Deactivated
                  </button>
                  <button
                    class="w-full text-left px-2 py-1.5 text-sm hover:bg-muted cursor-pointer transition-colors"
                    @mousedown.prevent="appendSearchToken('state:locked ')"
                  >
                    Locked
                  </button>
                </div>
...
<script setup lang="ts">
  ...
  import { buildUserCreateRoute, isUserSchemaType } from '@/console/utils/user-routes'
  ...
  const label = computed(
    () =>
      schemaDisplay.value.alias ||
      props.schemaType.replace(/_/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase()) + 's',
  )
  const singularLabel = computed(
    () => schemaDisplay.value.singular || label.value.replace(/s$/, '').replace(/ie$/, 'y'),
  )
  const createRoute = computed(() =>
    isUserSchemaType(props.schemaType)
      ? buildUserCreateRoute(props.schemaType)
      : `/s/${props.schemaType}/new`,
  )
  ...
  function applySearchQuery(query: string, table: any) {
    ...
    for (const token of tokens) {
      if (token.includes(':') && !token.startsWith('::')) {
        ...
        else if (['identifier', 'email', 'id'].includes(key))
          filters.push({ id: 'identifier', value })
  ...
  const columns = computed(() => {
    ...
    const cols: any[] = [
      ...
      columnHelper.accessor(
        (row) =>
          getField(row, 'client_name') || getField(row, 'display_name') || row.display_name || '—',
        {
          id: 'display_name',
          header: ({ column }) =>
            h(
        ...
        columnHelper.display({
          ...
          cell: ({ row }) =>
            h(
              ...
              () => getField(row.original, 'app_type') || '—',
            ),
        }),
        columnHelper.display({
          id: 'redirect_uris',
