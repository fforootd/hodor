      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem as-child>
            <router-link :to="userCreateRoute">
              <Users class="mr-2 size-4" />
              New User
            </router-link>
          </DropdownMenuItem>
          <DropdownMenuItem as-child>
            <router-link :to="serviceAccountCreateRoute">
              <KeyRound class="mr-2 size-4" />
              New Service Account
            </router-link>
          </DropdownMenuItem>
          <DropdownMenuItem as-child>
            <router-link :to="aiAgentCreateRoute">
    ...
    <Tabs :default-value="activeTab" @update:model-value="(val: any) => (activeTab = String(val))">
      <TabsList>
        ...
        <TabsTrigger value="human_user">
          ...
          Users
          <Badge
            v-if="typeCounts.human_user"
            variant="secondary"
            class="ml-1.5 text-xs px-1.5 py-0"
            >{{ typeCounts.human_user }}</Badge
          >
        </TabsTrigger>
        <TabsTrigger value="service_user">
          ...
          Service Accounts
          <Badge
            v-if="typeCounts.service_user"
            variant="secondary"
            class="ml-1.5 text-xs px-1.5 py-0"
            >{{ typeCounts.service_user }}</Badge
          >
        </TabsTrigger>
        <TabsTrigger value="ai_agent">
          ...
          AI Agents
          <Badge
            v-if="typeCounts.ai_agent"
            variant="secondary"
            class="ml-1.5 text-xs px-1.5 py-0"
            >{{ typeCounts.ai_agent }}</Badge
          >
        </TabsTrigger>
      </TabsList>
    </Tabs>
...
<script setup lang="ts">
  ...
  const userCreateRoute = buildUserCreateRoute('human_user')
  const serviceAccountCreateRoute = buildUserCreateRoute('service_user')
  const aiAgentCreateRoute = buildUserCreateRoute('ai_agent')
    ...
    columnHelper.accessor((row) => row._schemaType || 'unknown', {
      ...
      cell: (info) => {
        ...
        return h(
          ...
          {
            variant: 'outline',
            class: `font-normal flex items-center gap-1 ${cls} capitalize whitespace-nowrap`,
          },
          () => [h(Icon, { class: 'w-3 h-3 shrink-0' }), h('span', label)],
        )
      },
      enableHiding: true,
    }),
