-- +goose Up
-- Add category column to events table for fast event type filtering.
-- Categories: entity, auth, session, token, request, log, signal, threat, system.
ALTER TABLE events ADD COLUMN category TEXT NOT NULL DEFAULT '';

-- Backfill existing rows from event_type prefix.
...
-- Rename api.request → request.api.
...
-- SQLite doesn't support DROP COLUMN; category will remain but be unused.
