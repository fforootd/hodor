...
"$ROOT_DIR/browser-tests/scripts/preflight.sh"
...
wait_for_url() {
  ...
  for _ in $(seq 1 240); do
    if curl --silent --fail "$target" >/dev/null 2>&1; then
      echo "[browser-tests] ${label} ready: ${target}"
      return 0
    fi
    sleep 0.5
  done

  echo "[browser-tests] ${label} failed to become ready: ${target}" >&2
  return 1
}
...
"$ROOT_DIR/browser-tests/scripts/start-mock-oidc.sh" &
MOCK_PID=$!

"$ROOT_DIR/browser-tests/scripts/start-zitadel.sh" &
ZITADEL_PID=$!
...
if ! kill -0 "$MOCK_PID" >/dev/null 2>&1; then
  echo "[browser-tests] mock oidc exited unexpectedly" >&2
fi
if ! kill -0 "$ZITADEL_PID" >/dev/null 2>&1; then
  echo "[browser-tests] zitadel exited unexpectedly" >&2
fi
